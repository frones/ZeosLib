...
function ZAnsiRecToUnicode(Value: TZAnsiRec; CP: Word): ZWideString; {$IF defined(WITH_INLINE) and not defined(WITH_LCONVENCODING)}inline; {$IFEND}
...
{$ENDIF}
function ZConvertAnsiRecToUTF8(Const Src: TZAnsiRec; const RawCP: Word): UTF8String;
...
function ZMoveAnsiRecToUTF8(const Src: TZAnsiRec; const {%H-}RawCP: Word): UTF8String;
...
function ZDetectUTF8Encoding(Source: PAnsiChar; Len: Cardinal): TEncodeType; overload;
function ZDetectUTF8Encoding(const Source: RawByteString): TEncodeType; overload;
function USASCII7ToUnicodeString(Source: PAnsiChar; Len: Cardinal): ZWideString; overload;
...
begin
  case CP of
    ...
    28592:  //ISO_8859_2
      Result := UTF8Decode(ISO_8859_2ToUTF8(S));
    1250: //WIN1250
      Result := UTF8Decode(CP1250ToUTF8(S));
    1251: //WIN1251
      Result := UTF8Decode(CP1251ToUTF8(S));
    1252: //WIN1252
      Result := UTF8Decode(CP1252ToUTF8(S));
    1253: //WIN1253
      Result := UTF8Decode(CP1253ToUTF8(S));
    1254: //WIN1254
      Result := UTF8Decode(CP1254ToUTF8(S));
    1255: //WIN1255
      Result := UTF8Decode(CP1255ToUTF8(S));
    1256: //WIN1256
      Result := UTF8Decode(CP1256ToUTF8(S));
    1257: //WIN1257
      Result := UTF8Decode(CP1257ToUTF8(S));
    1258: //WIN1258
      Result := UTF8Decode(CP1258ToUTF8(S));
    437: //CP437
    ...
    874: //CP874
      Result := UTF8Decode(CP874ToUTF8(S));
    20866: //KOI8 (Russian)
      Result := UTF8Decode(KOI8ToUTF8(S));
    65001: //UTF8
      Result := UTF8Decode(S);
    else
      Result := ZUnknownRawToUnicodeWithAutoEncode(S, CP); //nearest result
  end;
end;
{$ELSE}
var
...
begin
  ...
  begin
    ...
    AnsiRec.P := PAnsiChar(S);
    Result := ZAnsiRecToUnicode(AnsiRec, CP);
    {$ELSE}
...
function ZAnsiRecToUnicode(Value: TZAnsiRec; CP: Word): ZWideString;
{$IFDEF WITH_LCONVENCODING}
var
  S: String;
begin
  SetString(S, Value.P, Value.Len);
...
begin
  ...
  if Value.Len = 0 then
    ...
    if CP = zCP_NONE  then
      case ZDetectUTF8Encoding(Value.P, Value.Len) of
        etUSASCII: Result := USASCII7ToUnicodeString(Value.P, Value.Len);
        etUTF8:
          begin
            CP := zCP_UTF8;
            goto A2U;
...
A2U:  {$IF defined(MSWINDOWS) or defined(WITH_UNICODEFROMLOCALECHARS)}
      begin
        ...
        while (Value.Len >= 4) and (PLongWord(Value.P)^ and $80808080 = 0) do
        begin
          C := PLongWord(Value.P)^;
          dec(Value.Len,4);
          inc(Value.P,4);
        ...
        while (Value.Len > 0) and (PByte(Value.P)^ and $80 = 0) do
        begin
          dec(Value.Len);
          PWord(Dest)^ := Byte(Value.P^); //Shift Byte to Word
          inc(Value.P);
          inc(Dest);
        end;
        if Value.Len > 0 then //convert remaining characters with codepage agnostic
        begin
          wlen :=  wlen-LengthInt(Value.Len); //elimainate already processed chars
          {$IFDEF WITH_UNICODEFROMLOCALECHARS}
          wlen := wlen + UnicodeFromLocaleChars(CP, 0, Value.P, Value.Len, Dest, Value.Len);
          {$ELSE}
          Wlen := wlen + MultiByteToWideChar(CP, 0, Value.P, Value.Len, Dest, Value.Len); //Convert Ansi to Wide with supported Chars
          {$ENDIF}
...
begin
  ...
  if Value.Len = 0 then
    ...
    {$IF defined(MSWINDOWS) or defined(WITH_UNICODEFROMLOCALECHARS)}
    begin
      ULen := Min(Integer(Value.Len) * 4, High(Integer)-1);
...
begin
  if (Src.P = nil) or (Src.Len = 0) then
    Result := ''
  else
  begin
    US := ZAnsiRecToUnicode(Src, RawCP);
...
function ZMoveAnsiRecToUTF8(const Src: TZAnsiRec; const RawCP: Word): UTF8String;
begin
  if (Src.P = nil) or (Src.Len = 0) then
    Result := ''
  else
    ZSetString(Src.P, Src.Len, Result);
end;
...
begin
  ...
  ConSettings^.ConvFuncs.ZAnsiRecToUTF8 := nil;
  ...
  if ZCompatibleCodePages(ConSettings^.ClientCodePage^.CP, zCP_UTF8) and
    ...
    ConSettings^.ConvFuncs.ZAnsiRecToUTF8 := @ZMoveAnsiRecToUTF8
  else
    ConSettings^.ConvFuncs.ZAnsiRecToUTF8 := @ZConvertAnsiRecToUTF8;
...
function ZDetectUTF8Encoding(Source: PAnsiChar; Len: Cardinal): TEncodeType;
var
  c : Byte;
  EndPtr: PAnsichar;
begin
...
{EH:
partial copy of SynCommons.pas TSynAnsiConvert.AnsiBufferToUnicode
original autor: Arnaud Bouchez
Changes: NO Check if SourceBytes < #127 and NO WideChar casts -> horrable slow with FPC
}
function USASCII7ToUnicodeString(Source: PAnsiChar; Len: Cardinal): ZWideString; overload;
var C: LongWord;
  Dest: PWideChar;
...
begin
  ...
  begin
    ...
    Result := ZUnicodeToRaw(ZAnsiRecToUnicode(AnsiRec, MsgCP), RawCP);
  end;
end;
