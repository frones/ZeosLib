...
function TZAdoStatement.ExecuteQuery(const SQL: ZAnsiString): IZResultSet;
begin
  if ASQL <> SQL then
    ASQL := SQL;
...
function TZAdoStatement.ExecuteUpdate(const SQL: ZAnsiString): Integer;
begin
  if ASQL <> SQL then
    ASQL := SQL;
...
function TZAdoStatement.Execute(const SQL: ZAnsiString): Boolean;
begin
  if ASQL <> SQL then
    ASQL := SQL;
  Result := Execute(WSQL);
...
begin
  ...
  begin
    ...
    case ConvertAdoToSqlType(FAdoCommand.Parameters.Item[ParameterIndex - 1].Type_, ConSettings.CPType) of
      ...
      stBytes:
        DefVarManager.SetAsString(Result, String(BytesToStr(VarToBytes(Temp))));
