...
function TZAbstractPreparedStatement2.ExecuteQuery(const SQL: ZWideString): IZResultSet;
begin
  WSQL := SQL;
  Result := ExecuteQueryPrepared;
end;
...
function TZAbstractPreparedStatement2.ExecuteUpdate(const SQL: ZWideString): Integer;
begin
  WSQL := SQL;
  Result := ExecuteUpdatePrepared;
end;
...
function TZAbstractPreparedStatement2.ExecuteQuery(const SQL: RawByteString): IZResultSet;
begin
  ASQL := SQL;
  Result := ExecuteQueryPrepared;
end;
...
function TZAbstractPreparedStatement2.ExecuteUpdate(const SQL: RawByteString): Integer;
begin
  ASQL := SQL;
  Result := ExecuteUpdatePrepared;
end;
...
function TZAbstractPreparedStatement2.Execute(const SQL: RawByteString): Boolean;
begin
  ASQL := SQL;
  Result := ExecutePrepared;
end;
...
function TZAbstractPreparedStatement2.GetClientVariantManger: IZClientVariantManager;
begin
...
procedure TZAbstractPreparedStatement2.SetInParamCount(const NewParamCount: Integer);
var
  I: Integer;
begin
...
procedure TZAbstractPreparedStatement2.SetInParam(ParameterIndex: Integer;
...
procedure TZAbstractPreparedStatement2.LogPrepStmtMessage(Category: TZLoggingCategory;
...
function TZAbstractPreparedStatement2.GetInParamLogValue(Value: TZVariant): RawByteString;
begin
  With Value do
    case VType of
      vtNull : result := '(NULL)';
...
procedure TZAbstractPreparedStatement2.SetDefaultValue(
...
procedure TZAbstractPreparedStatement2.SetInt(ParameterIndex: Integer;
  Value: Integer);
begin
...
procedure TZAbstractPreparedStatement2.SetBigDecimal(
...
procedure TZAbstractPreparedStatement2.SetCharRec(ParameterIndex: Integer;
  const Value: TZCharRec);
begin
...
procedure TZAbstractPreparedStatement2.SetString(ParameterIndex: Integer;
   const Value: String);
begin
...
{$IFNDEF NO_ANSISTRING}
procedure TZAbstractPreparedStatement2.SetAnsiString(ParameterIndex: Integer;
   const Value: AnsiString);
begin
...
{$IFNDEF NO_UTF8STRING}
procedure TZAbstractPreparedStatement2.SetUTF8String(ParameterIndex: Integer;
   const Value: UTF8String);
begin
...
procedure TZAbstractPreparedStatement2.SetRawByteString(ParameterIndex: Integer;
   const Value: RawByteString);
begin
...
procedure TZAbstractPreparedStatement2.SetUnicodeString(ParameterIndex: Integer;
  const Value: ZWideString);
begin
...
procedure TZAbstractPreparedStatement2.SetBytes(ParameterIndex: Integer; Value: PByte; Len: NativeUInt);
var
  Bytes: TBytes;
begin
  SetLength(Bytes, Len);
...
procedure TZAbstractPreparedStatement2.SetGUID(ParameterIndex: Integer; const Value: TGUID);
begin
...
procedure TZAbstractPreparedStatement2.SetDate(ParameterIndex: Integer; const Value: TZDate);
begin
...
procedure TZAbstractPreparedStatement2.SetTime(ParameterIndex: Integer; const Value: TZTime);
begin
...
procedure TZAbstractPreparedStatement2.SetTimestamp(ParameterIndex: Integer; const Value: TZTimeStamp);
begin
...
procedure TZAbstractPreparedStatement2.SetAsciiStream(
...
procedure TZAbstractPreparedStatement2.SetUnicodeStream(
  ParameterIndex: Integer; const Value: TStream);
begin
  if TMemoryStream(Value).Memory = nil
...
procedure TZAbstractPreparedStatement2.SetBinaryStream(
  ParameterIndex: Integer; const Value: TStream);
begin
...
function TZAbstractPreparedStatement2.GetBoolean(ParameterIndex: Integer): Boolean;
begin
...
function TZAbstractPreparedStatement2.GetShort(ParameterIndex: Integer): ShortInt;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
  Result := SoftVarManager.GetAsInteger(InParamValues[ParameterIndex - FirstDbcIndex]);
...
function TZAbstractPreparedStatement2.GetSmall(ParameterIndex: Integer): SmallInt;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
  Result := SoftVarManager.GetAsInteger(InParamValues[ParameterIndex - FirstDbcIndex]);
...
function TZAbstractPreparedStatement2.GetUInt(ParameterIndex: Integer): Cardinal;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
function TZAbstractPreparedStatement2.GetDouble(ParameterIndex: Integer): Double;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
  Result := SoftVarManager.GetAsDouble(InParamValues[ParameterIndex - FirstDbcIndex]);
...
function TZAbstractPreparedStatement2.GetCurrency(ParameterIndex: Integer): Currency;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
procedure TZAbstractPreparedStatement2.GetBigDecimal(ParameterIndex: Integer; var Result: TBCD);
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
procedure TZAbstractPreparedStatement2.GetGUID(Index: Integer; var Result: TGUID);
begin
  if (Index - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
function TZAbstractPreparedStatement2.GetDate(ParameterIndex: Integer): TDateTime;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
procedure TZAbstractPreparedStatement2.GetDate(ParameterIndex: Integer; Var Result: TZDate);
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
function TZAbstractPreparedStatement2.GetTime(ParameterIndex: Integer): TDateTime;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
  Result := SoftVarManager.GetAsDateTime(InParamValues[ParameterIndex - FirstDbcIndex]);
...
procedure TZAbstractPreparedStatement2.GetTime(ParameterIndex: Integer; Var Result: TZTime);
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
function TZAbstractPreparedStatement2.GetTimestamp(ParameterIndex: Integer): TDateTime;
var
  Res: TZTime;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
...
procedure TZAbstractPreparedStatement2.GetTimeStamp(Index: Integer; var Result: TZTimeStamp);
begin
  if (Index - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
function TZAbstractPreparedStatement2.GetValue(ParameterIndex: Integer): TZVariant;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
function TZAbstractPreparedStatement2.GetString(ParameterIndex: Integer): String;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
{$IFNDEF NO_ANSISTRING}
function TZAbstractPreparedStatement2.GetAnsiString(ParameterIndex: Integer): AnsiString;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
{$IFNDEF NO_UTF8STRING}
function TZAbstractPreparedStatement2.GetUTF8String(ParameterIndex: Integer): UTF8String;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
function TZAbstractPreparedStatement2.GetRawByteString(ParameterIndex: Integer): RawByteString;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
function TZAbstractPreparedStatement2.GetUnicodeString(ParameterIndex: Integer): ZWideString;
begin
  if (ParameterIndex - FirstDbcIndex) >= Length(InParamValues) then
    raise EZSQLException.Create('Paramter index exceeds parameter count.');
...
function TZAbstractPreparedStatement2.GetBLob(ParameterIndex: Integer): IZBlob;
var
  Idx: Integer;
  TempBlob: IZBlob;
begin
...
procedure TZAbstractPreparedStatement2.AddBatchPrepared;
begin
...
function TZAbstractPreparedStatement2.GetMetaData: IZResultSetMetaData;
begin
  Result := nil;
  RaiseUnsupportedException;
end;
...
procedure TZAbstractPreparedStatement2.RaiseUnsupportedException;
begin
...
function TZAbstractPreparedStatement2.GetOmitComments: Boolean;
begin
  Result := False;
end;

function TZAbstractPreparedStatement2.GetCompareFirstKeywordStrings: TPreparablePrefixTokens;
begin
  Result := nil;
end;
