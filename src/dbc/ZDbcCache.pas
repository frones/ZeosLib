...
type
  ...
  public
    ...
    function GetPRaw(Const ColumnIndex: Integer; var IsNull: Boolean; out Len: NativeUInt): PAnsiChar; virtual;
    ...
    procedure SetPRaw(Const ColumnIndex: Integer; Value: PAnsiChar; Len: PNativeUInt);virtual;
  ...
  public
    ...
    function GetPRaw(Const ColumnIndex: Integer; var IsNull: Boolean; out Len: NativeUInt): PAnsiChar; override;
    ...
    procedure SetPRaw(Const ColumnIndex: Integer; Value: PAnsiChar; Len: PNativeUInt); override;
...
begin
  ...
  begin
    ...
    case FColumnTypes[ColumnIndex] of
      ...
      stWord:
        Result := Ord((PWord(ValuePtr1)^ > PWord(ValuePtr2)^))-Ord((PWord(ValuePtr1)^ < PWord(ValuePtr2)^));
      stSmall:
        Result := Ord((PSmallInt(ValuePtr1)^ > PSmallInt(ValuePtr2)^))-Ord((PSmallInt(ValuePtr1)^ < PSmallInt(ValuePtr2)^));
      stLongWord:
        Result := Ord((PLongWord(ValuePtr1)^ > PLongWord(ValuePtr2)^))-Ord((PLongWord(ValuePtr1)^ < PLongWord(ValuePtr2)^));
      stInteger:
        Result := Ord((PLongInt(ValuePtr1)^ > PLongInt(ValuePtr2)^))-Ord((PLongInt(ValuePtr1)^ < PLongInt(ValuePtr2)^));
      stULong:
        Result := Ord((PUInt64(ValuePtr1)^ > PUInt64(ValuePtr2)^))-Ord((PUInt64(ValuePtr1)^ < PUInt64(ValuePtr2)^));
      stLong:
      ...
      stCurrency:
        Result := Ord((PCurrency(ValuePtr1)^ > PCurrency(ValuePtr2)^))-Ord((PCurrency(ValuePtr1)^ < PCurrency(ValuePtr2)^));
      stBigDecimal:
      ...
      stBytes,stGUID:
        begin
          ...
          if Result = 0 then
          begin
            Bts1 := InternalGetBytes(Buffer1, ColumnIndex{$IFNDEF GENERIC_INDEX}+1{$ENDIF});
            Bts2 := InternalGetBytes(Buffer2, ColumnIndex{$IFNDEF GENERIC_INDEX}+1{$ENDIF});
      ...
      stAsciiStream, stBinaryStream, stUnicodeStream:
        begin
          Blob1 := GetBlobObject(Buffer1, ColumnIndex{$IFNDEF GENERIC_INDEX} + 1{$ENDIF});
          BlobEmpty1 := (Blob1 = nil) or (Blob1.IsEmpty);
          Blob2 := GetBlobObject(Buffer2, ColumnIndex{$IFNDEF GENERIC_INDEX} + 1{$ENDIF});
...
function TZRowAccessor.GetPRaw(Const ColumnIndex: Integer; var IsNull: Boolean;
...
procedure TZRowAccessor.SetPRaw(Const ColumnIndex: Integer; Value: PAnsichar;
  Len: PNativeUInt);
var
  IsNull: Boolean;
  GUID: TGUID;
...
begin
  case FColumnTypes[ColumnIndex{$IFNDEF GENERIC_INDEX} - 1{$ENDIF}] of
    ...
    stULong: PUInt64(@FBuffer.Columns[FColumnOffsets[ColumnIndex{$IFNDEF GENERIC_INDEX} - 1{$ENDIF}] + 1])^ := RawToInt64Def(Value, 0);
    stLong: PInt64(@FBuffer.Columns[FColumnOffsets[ColumnIndex{$IFNDEF GENERIC_INDEX} - 1{$ENDIF}] + 1])^ := RawToInt64Def(Value, 0);
    stFloat: PSingle(@FBuffer.Columns[FColumnOffsets[ColumnIndex{$IFNDEF GENERIC_INDEX} - 1{$ENDIF}] + 1])^ := SQLStrToFloatDef(Value, 0);
    stDouble: PDouble(@FBuffer.Columns[FColumnOffsets[ColumnIndex{$IFNDEF GENERIC_INDEX} - 1{$ENDIF}] + 1])^ := SQLStrToFloatDef(Value, 0);
    stCurrency: PCurrency(@FBuffer.Columns[FColumnOffsets[ColumnIndex{$IFNDEF GENERIC_INDEX} - 1{$ENDIF}] + 1])^ := SQLStrToFloatDef(Value, 0);
    stBigDecimal: PExtended(@FBuffer.Columns[FColumnOffsets[ColumnIndex{$IFNDEF GENERIC_INDEX} - 1{$ENDIF}] + 1])^ := SQLStrToFloatDef(Value, 0);
    //stString, stUnicodeString: do not handle here!
    stBytes: SetBytes(ColumnIndex, StrToBytes(Value));
    stGUID:
      if Value = '' then
    ...
    vtCharRec:
      ...
      begin
        ...
        begin
          ...
          SetPRaw(ColumnIndex, Value.VCharRec.P, @Len)
        end
        else
...
function TZRawRowAccessor.GetPRaw(Const ColumnIndex: Integer; var IsNull: Boolean;
  out Len: NativeUInt): PAnsiChar;
begin
{$IFNDEF DISABLE_CHECKING}
  CheckColumnConvertion(ColumnIndex, stString);
  ...
  begin
    case FColumnTypes[ColumnIndex{$IFNDEF GENERIC_INDEX} - 1{$ENDIF}] of
      stString, stUnicodeString:
        ...
        Result := inherited GetPRaw(ColumnIndex, IsNull, Len);
    end;
    IsNull := False;
  end
  else
...
var Len: NativeUint;
begin
  Result.P := GetPRaw(ColumnIndex, IsNull, Len);
  Result.Len := Len;
...
procedure TZRawRowAccessor.SetPRaw(Const ColumnIndex: Integer;
...
{$ENDIF}
  if Value = nil then
    ...
    case FColumnTypes[ColumnIndex{$IFNDEF GENERIC_INDEX} - 1{$ENDIF}] of
      ...
      else inherited SetPRaw(ColumnIndex, Value, Len)
    end;
end;
...
{$ENDIF}
  if Value.P = nil then
    ...
    case FColumnTypes[ColumnIndex{$IFNDEF GENERIC_INDEX} - 1{$ENDIF}] of
      stString, stUnicodeString:
        begin
          InternalSetString(FBuffer, ColumnIndex, PUnicodeToRaw(Value.P, Value.Len, ConSettings^.ClientCodePage^.CP));
          FBuffer.Columns[FColumnOffsets[ColumnIndex{$IFNDEF GENERIC_INDEX} - 1{$ENDIF}]] := bIsNotNull;
        end;
...
function TZUnicodeRowAccessor.GetPRaw(Const ColumnIndex: Integer; var IsNull: Boolean;
  out Len: NativeUInt): PAnsiChar;
begin
{$IFNDEF DISABLE_CHECKING}
  CheckColumnConvertion(ColumnIndex, stString);
...
procedure TZUnicodeRowAccessor.SetPRaw(Const ColumnIndex: Integer;
  Value: PAnsiChar; Len: PNativeUInt);
begin
{$IFNDEF DISABLE_CHECKING}
  CheckColumnConvertion(ColumnIndex, stString);
