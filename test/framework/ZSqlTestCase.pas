...
const
  ...
  pl_interbase_client_server = 'interbase,firebird,interbase-6,firebird-1.0,firebird-1.5,firebird-2.0,firebird-2.1,firebird-2.5,firebird-3.0';
...
type
  ...
  protected
    ...
    function GetDBTestString(const Value: ZWideString; ConSettings: PZConSettings; MaxLen: Integer = -1): {$IFDEF UNICODE}String{$ELSE}RawByteString{$ENDIF}; overload;
...
function TZAbstractSQLTestCase.GetDBTestString(const Value: ZWideString;
  ConSettings: PZConSettings; MaxLen: Integer = -1): {$IFDEF UNICODE}String{$ELSE}RawByteString{$ENDIF};
{$IFNDEF UNICODE}
var CP: Word;
{$ENDIF}
begin
