...
begin
  Result := 'mssql,sybase,FreeTDS_MsSQL<=6.5,FreeTDS_MsSQL-7.0,FreeTDS_MsSQL-2000,FreeTDS_MsSQL>=2005,FreeTDS_Sybase<10,FreeTDS_Sybase-10+,ado';
end;
...
begin
  ...
  Props.Values['preferprepared'] := 'False';
  Stmt := nil;
  RS := nil;
  try
...
begin
  Props := TStringList.Create;
  Props.Values['preferprepared'] := 'False'; //turn off the prepared mode
  PStmt := nil;
  Stmt := Connection.CreateStatementWithParams(Props);;
  RS := nil;
  try
